...
<body class="bg-slate-50 text-slate-900 min-h-screen flex flex-col antialiased selection:bg-teal-500 selection:text-white">
            ...
            <div class="bg-white p-5 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
              ...
              <div id="student-pdf-container-wrapper" class="hidden space-y-4">
                <div class="rounded-3xl border-2 border-indigo-200 bg-indigo-50/40 p-5 sm:p-6 space-y-4">
                  ...
                  <div class="relative w-full h-96 sm:h-[500px] rounded-2xl overflow-hidden border border-indigo-200 bg-slate-100 shadow-inner">
                    <object id="pdf-embed-object" data="" type="application/pdf" class="w-full h-full">
  ...
  <script>
            ...
            videoSteps: [
              {
                id: "vstep-1-1-1",
                format: "video",
                videoTitle: "Video Link 1: Core Principles of Strength-Based Mentoring",
                pdfTitle: "Neurodiversity & Strength-Based Mentoring Guide",
              ...
              },
              {
                id: "vstep-1-1-2",
                format: "video",
                videoTitle: "Video Link 2: Processing Pauses & Non-Verbal Cues",
                pdfTitle: "Processing Pauses & Non-Verbal Cues Protocol",
            ...
            videoSteps: [
              {
                id: "vstep-sub1-2-pdf",
                format: "pdf",
                videoTitle: "De-escalation & Sensory Toolkit Video Lesson",
                pdfTitle: "Inclusive Classrooms De-escalation Protocol & Sensory Regulation Guide",
                title: "Inclusive Classrooms De-escalation Protocol & Sensory Regulation Guide",
    ...
    // ==========================================
    // INDEXEDDB RESILIENT PDF STORAGE (NO 5MB LOCALSTORAGE QUOTA LIMITS)
    // ==========================================
    const IDB_NAME = "LCS_DOCUMENT_STORE_V2";
    ...
    const pdfBlobUrlCache = {}; // storageKey -> blobUrl

    async function storePdfBlob(key, blobOrFile) {
      try {
        ...
        console.warn("storePdfBlob error:", e);
        return false;
      }
    }
    ...
    async function restorePdfBlobs(dbArray) {
      if (!Array.isArray(dbArray)) return;
      for (const mod of dbArray) {
        if (!mod.submodules) continue;
        for (const sub of mod.submodules) {
          if (sub.pdfDoc && sub.pdfDoc.storageKey) {
            const blob = await getPdfBlob(sub.pdfDoc.storageKey);
            if (blob) {
              const url = URL.createObjectURL(blob);
              pdfBlobUrlCache[sub.pdfDoc.storageKey] = url;
              sub.pdfDoc.url = url;
            }
          }
          if (sub.videoSteps) {
            for (const st of sub.videoSteps) {
              if (st.pdfStorageKey) {
                const blob = await getPdfBlob(st.pdfStorageKey);
                if (blob) {
                  const url = URL.createObjectURL(blob);
                  pdfBlobUrlCache[st.pdfStorageKey] = url;
                  st.pdfUrl = url;
                }
              }
            }
    ...
    function sanitizeForStorage(val) {
      return JSON.parse(JSON.stringify(val, (k, v) => {
        // Strip out large data URLs or blob: URLs so localStorage never hits 5MB quota
        if (k === 'pdfUrl' && typeof v === 'string') {
          if (v.startsWith('blob:') || (v.startsWith('data:') && v.length > 50000)) {
            return ""; // Re-hydrated from IndexedDB
          }
        }
        return v;
      }));
    ...
    // Hydrate IndexedDB blobs asynchronously
    restorePdfBlobs(publishedDb);
    restorePdfBlobs(draftDb);
    ...
    function publishContent() {
      ...
      // Hydrate publishedDb with active blob URLs from cache
      for (const mod of publishedDb) {
        if (!mod.submodules) continue;
        for (const sub of mod.submodules) {
          if (sub.videoSteps) {
            for (const st of sub.videoSteps) {
              if (st.pdfStorageKey && pdfBlobUrlCache[st.pdfStorageKey]) {
                st.pdfUrl = pdfBlobUrlCache[st.pdfStorageKey];
              }
            }
          }
        }
      ...
      // If currently on companion view, re-render immediate content
      if (currentView === 'student') {
        if (selectedModuleId && selectedSubmodule && !document.getElementById('student-level-content').classList.contains('hidden')) {
          const pMod = publishedDb.find(m => m.id === selectedModuleId);
          const pSub = pMod ? pMod.submodules.find(s => s.id === selectedSubmodule.id) : null;
          if (pSub) {
            selectedSubmodule = pSub;
            renderSubmoduleContent(pSub, pMod);
            return;
          }
        }
        goHome();
    ...
    function initAuthorDesk() {
      ...
      if (mod) {
        ...
        // CRITICAL FIX: Ensure selectedSubmodule is ALWAYS the object inside draftDb (not publishedDb)
        const currentSubId = selectedSubmodule ? selectedSubmodule.id : (mod.submodules[0] ? mod.submodules[0].id : null);
        selectedSubmodule = mod.submodules.find(s => s.id === currentSubId) || mod.submodules[0];
    ...
    function updateStepTitle(vIdx, val, format) {
    ...
    function toggleStepFormat(vIdx, newFormat) {
      ...
      if (newFormat === 'video') {
        ...
        // Restore video title if present without using pdf name
        if (step.videoTitle) {
          step.title = step.videoTitle;
        } else if (!step.title || (step.pdfTitle && step.title === step.pdfTitle)) {
          step.title = `Video Lesson ${vIdx + 1}: Core Principles`;
          step.videoTitle = step.title;
        }
      } else if (newFormat === 'pdf') {
        if (!step.pdfUrl && !step.pdfStorageKey) {
        ...
        // Restore PDF title without using video title
        if (step.pdfTitle) {
          step.title = step.pdfTitle;
        } else if (!step.title || (step.videoTitle && step.title === step.videoTitle)) {
          step.title = step.pdfFileName ? step.pdfFileName.replace(/\.[^/.]+$/, "") : `PDF Reading Guide ${vIdx + 1}`;
          step.pdfTitle = step.title;
        }
      }
      markDraftDirty();
      renderAuthorContentFormatSteps();
          ...
          <details open class="group bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden transition-all shadow-sm">
            <summary class="p-4 sm:p-5 bg-white border-b border-slate-200 flex flex-wrap items-center justify-between gap-3 cursor-pointer list-none select-none">
              <div class="flex items-center gap-3">
                ...
                <span class="text-xs sm:text-sm font-extrabold text-slate-900">${isPdf ? (vStep.pdfTitle || vStep.title) : (vStep.videoTitle || vStep.title)}</span>
              </div>
              <div class="flex items-center gap-2">
            ...
            <div class="p-5 sm:p-6 space-y-5">
              ...
              <!-- Step Title -->
              <div>
                <label class="block text-[10px] font-black uppercase text-slate-400 mb-1">${isPdf ? '📄 PDF Document Guide Title' : '📹 Video Lesson Title'}</label>
                <input type="text" value="${isPdf ? (vStep.pdfTitle || vStep.title) : (vStep.videoTitle || vStep.title)}" oninput="updateStepTitle(${vIdx}, this.value, '${isPdf ? 'pdf' : 'video'}')" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold bg-white" />
              </div>
    ...
    async function processAuthorPDFFile(file, vIdx) {
      if (!file) return;
      ...
      // Explicitly set format to 'pdf' when a PDF is uploaded to this step
      step.format = 'pdf';

      // 1. Store raw file/blob in IndexedDB (handles 15MB+ with zero quota errors)
      const storageKey = `pdf_${selectedSubmodule.id}_step_${step.id || vIdx}_${Date.now()}`;
      await storePdfBlob(storageKey, file);

      // 2. Generate native Blob URL (Chrome renders blob URLs in iframe/object without blank screens)
      const blobUrl = URL.createObjectURL(file);
      pdfBlobUrlCache[storageKey] = blobUrl;

      step.pdfStorageKey = storageKey;
      step.pdfUrl = blobUrl;
      ...
      // 3. Set PDF title without modifying videoTitle!
      step.pdfTitle = file.name.replace(/\.[^/.]+$/, "");
      step.title = step.pdfTitle;

      markDraftDirty();
      renderAuthorContentFormatSteps();
      showToast(`Uploaded "${file.name}" (${step.pdfFileSize}) successfully! Attached to this step.`, 'emerald');
    }
      ...
      container.innerHTML = quiz.map((q, idx) => {
        ...
        return `
          <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
            ...
            ${isSubj ? `
              <input type="text" value="${q.prompt}" oninput="draftDb.find(m=>m.id==='${mod.id}').masterQuiz[${idx}].prompt = this.value; markDraftDirty()" class="w-full rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-bold bg-white" placeholder="Prompt..." />
              <textarea oninput="draftDb.find(m=>m.id==='${mod.id}').masterQuiz[${idx}].exemplar = this.value; markDraftDirty()" rows="2" class="w-full rounded-xl border border-slate-200 p-2 text-xs bg-white" placeholder="Exemplar answer...">${q.exemplar || ''}</textarea>
            ` : `
              <input type="text" value="${q.question}" oninput="draftDb.find(m=>m.id==='${mod.id}').masterQuiz[${idx}].question = this.value; markDraftDirty()" class="w-full rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-bold bg-white" placeholder="Question..." />
            `}
          </div>
        `;
      }).join('');
    ...
    function openSubmodule(subId) {
      let mod = publishedDb.find(m => m.id === selectedModuleId);
      if (!mod || !mod.submodules.some(s => s.id === subId)) {
        mod = publishedDb.find(m => m.submodules && m.submodules.some(s => s.id === subId)) || publishedDb[0];
        if (mod) selectedModuleId = mod.id;
      }
      const sub = mod ? mod.submodules.find(s => s.id === subId) : null;
      if (!sub) return;
    ...
    // =========================================================================
    let activePdfMilestoneIndexMap = {}; // submoduleId -> number (0-indexed current milestone)
    ...
    function renderStudentPDFView(step) {
      ...
      const embedObj = document.getElementById('pdf-embed-object');
      const checkpointsList = document.getElementById('pdf-checkpoints-list');

      // Hydrate from blob cache if key exists
      if (step.pdfStorageKey && pdfBlobUrlCache[step.pdfStorageKey]) {
        step.pdfUrl = pdfBlobUrlCache[step.pdfStorageKey];
      }
      ...
      const fileName = step.pdfFileName || (step.pdfTitle ? step.pdfTitle + ".pdf" : (pdfUrl.split('/').pop().split('?')[0] || "guide.pdf"));
      if (titleEl) titleEl.innerText = step.pdfTitle || step.title || fileName.replace(/\.[^/.]+$/, "");
      if (downloadBtn) {
        downloadBtn.href = pdfUrl;
        downloadBtn.download = fileName;
      }
      if (embedFrame) embedFrame.src = pdfUrl + "#toolbar=0";
      if (embedObj) embedObj.data = pdfUrl + "#toolbar=0";

      const checkpoints = step.pdfCheckpoints || [];
      if (!checkpointsList) return;

      if (checkpoints.length === 0) {
        checkpointsList.innerHTML = `<div class="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100 text-xs font-bold text-slate-600">No embedded checkpoints for this reading.</div>`;
        unlockPDFReflections();
        return;
      }

      const subId = selectedSubmodule.id;
      if (activePdfMilestoneIndexMap[subId] === undefined) {
        // default to first unpassed checkpoint
        const firstUnpassed = checkpoints.findIndex((_, idx) => !pdfCheckpointsCompletedMap[`${subId}_pdf_${idx}`]);
        activePdfMilestoneIndexMap[subId] = firstUnpassed === -1 ? checkpoints.length - 1 : firstUnpassed;
      }
      let currentMilestone = activePdfMilestoneIndexMap[subId];
      if (currentMilestone >= checkpoints.length) {
        currentMilestone = checkpoints.length - 1;
      }

      const allPdfCheckpointsPassed = checkpoints.every((_, idx) => pdfCheckpointsCompletedMap[`${subId}_pdf_${idx}`]);
      if (allPdfCheckpointsPassed) {
        unlockPDFReflections();
      }

      const completedCount = checkpoints.filter((_, idx) => pdfCheckpointsCompletedMap[`${subId}_pdf_${idx}`]).length;
      const progressPct = Math.round((completedCount / checkpoints.length) * 100);

      let html = `
        <div class="space-y-4">
          <!-- Sequential Milestone Stepper Bar -->
          <div class="p-4 bg-indigo-50/70 rounded-2xl border border-indigo-200 space-y-2.5 shadow-sm">
            <div class="flex items-center justify-between text-xs font-black uppercase text-indigo-950">
              <span class="flex items-center gap-1.5">
                <span>⏱️</span> <span>Reading Milestones Progression</span>
              </span>
              <span class="text-indigo-700 font-extrabold">
                ${allPdfCheckpointsPassed ? '✓ All Completed (100%)' : `Milestone ${currentMilestone + 1} of ${checkpoints.length} (${progressPct}%)`}
              </span>
            </div>

            <!-- Progress bar -->
            <div class="w-full bg-indigo-200/60 h-2.5 rounded-full overflow-hidden">
              <div class="bg-indigo-600 h-full rounded-full transition-all duration-500" style="width: ${progressPct}%"></div>
            </div>

            <!-- Milestone Step Badges -->
            <div class="flex flex-wrap items-center gap-2 pt-1">
              ${checkpoints.map((cp, idx) => {
                const isPassed = pdfCheckpointsCompletedMap[`${subId}_pdf_${idx}`];
                const isCurrent = idx === currentMilestone && !allPdfCheckpointsPassed;
                let badgeClass = "px-3 py-1 rounded-xl text-xs font-extrabold flex items-center gap-1.5 transition-all ";
                if (isPassed) {
                  badgeClass += "bg-emerald-100 text-emerald-800 border border-emerald-300";
                } else if (isCurrent) {
                  badgeClass += "bg-indigo-600 text-white shadow-sm ring-2 ring-indigo-300";
                } else {
                  badgeClass += "bg-slate-100 text-slate-400 border border-slate-200 opacity-60";
                }
                const shortLabel = cp.section ? cp.section.split(':')[0] : `Milestone ${idx + 1}`;
                return `
      ...
      if (allPdfCheckpointsPassed) {
        // Celebration Banner
        html += `
          <div class="p-5 bg-gradient-to-r from-emerald-600 to-teal-700 text-white rounded-2xl shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
              <span class="text-3xl">🎉</span>
              <div>
                <h5 class="text-sm font-black">All Reading Milestones Mastered!</h5>
                <p class="text-xs opacity-90 mt-0.5">You have verified your comprehension across all sections of this protocol. The reflections below are unlocked.</p>
              </div>
            </div>
            <button type="button" onclick="activePdfMilestoneIndexMap['${subId}'] = 0; renderStudentPDFView(selectedSubmodule.videoSteps[activeVideoStepIndex] || selectedSubmodule);" class="px-4 py-2 bg-white/20 hover:bg-white/30 text-white rounded-xl text-xs font-bold whitespace-nowrap self-start sm:self-auto border border-white/30">
              ↺ Review Questions
            </button>
          </div>
        `;
      } else {
        // Active Milestone Gated Card
        const cp = checkpoints[currentMilestone];
        const key = `${subId}_pdf_${currentMilestone}`;
        const hasAnswered = pdfCheckpointsCompletedMap[key];
        const selected = selectedChoicesMap[key];
        const isCorrect = selected !== undefined && cp.correctIndex === selected;

        html += `
          <div class="p-5 sm:p-6 bg-white rounded-2xl border-2 border-indigo-200 shadow-sm space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-indigo-100 pb-3">
              <div class="flex items-center gap-2">
                <span class="px-2.5 py-1 rounded-xl bg-indigo-600 text-white text-[10px] font-black uppercase tracking-wider">
                  Active Milestone ${currentMilestone + 1} of ${checkpoints.length}
                </span>
                <span class="text-xs font-black text-indigo-950">${cp.section || `Milestone ${currentMilestone + 1}`}</span>
              </div>
              <span class="text-xs font-bold ${hasAnswered ? 'text-emerald-600' : 'text-amber-600'} flex items-center gap-1">
                <span>${hasAnswered ? '✓ Mastered' : '⏱️ Reading in Progress'}</span>
              </span>
            </div>

            <!-- Target Reading Section Notice -->
            <div class="p-3.5 bg-indigo-50/70 rounded-xl border border-indigo-100 text-xs text-indigo-950 flex items-start gap-2.5">
              <span class="text-lg flex-shrink-0">📖</span>
              <div>
                <span class="font-black uppercase text-[10px] text-indigo-800">Target Document Section:</span>
                <p class="font-bold text-slate-800 mt-0.5">${cp.section || 'Read this section in the PDF document above before answering:'}</p>
              </div>
            </div>

            <!-- Question Prompt -->
            <div class="space-y-1">
              <h4 class="text-sm font-extrabold text-slate-900">${cp.question}</h4>
              <p class="text-[11px] text-slate-500">Select the correct response according to the reading material to proceed.</p>
            </div>
            ...
            <div class="space-y-2 pt-1">
              ${(cp.choices || []).map((ch, chIdx) => {
                ...
                const isChoiceCorrect = cp.correctIndex === chIdx;
                let optClass = "p-3 rounded-xl border text-xs font-bold cursor-pointer transition-all flex items-center justify-between ";
                if (hasAnswered) {
                  if (isChoiceCorrect) optClass += "bg-emerald-50 border-emerald-400 text-emerald-900 shadow-sm";
                  ...
                  optClass += "bg-slate-50 border-slate-200 hover:bg-indigo-50/70 hover:border-indigo-300 text-slate-700";
                }

                return `
                  <div onclick="submitPDFSequentialAnswer('${key}', ${currentMilestone}, ${chIdx})" class="${optClass}">
                    <span>${ch}</span>
                    ${hasAnswered && isChoiceCorrect ? '<span class="text-emerald-600 font-black">✓ Correct</span>' : ''}
                    ${hasAnswered && isSelected && !isChoiceCorrect ? '<span class="text-rose-600 font-black">✕ Try Again</span>' : ''}
                  </div>
                `;
              }).join('')}
            </div>

            <!-- Feedback & Next Action Gate -->
            ${hasAnswered ? `
              <div class="pt-3 border-t border-indigo-100 space-y-3">
                <div class="p-3.5 rounded-xl bg-teal-50 border border-teal-200 text-xs text-teal-900 space-y-1">
                  <span class="font-extrabold uppercase text-[10px] text-teal-800">Author Exemplar Explanation:</span>
                  <p>${cp.exemplar || cp.feedbacks?.[cp.correctIndex] || "Mastered! Low-demand and emotional grounding precede de-escalation."}</p>
                </div>

                <div class="flex justify-end pt-1">
                  <button type="button" onclick="advancePDFMilestone(${currentMilestone + 1})" class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black shadow-md transition flex items-center gap-1.5 animate-pulse">
                    <span>${currentMilestone + 1 < checkpoints.length ? `Unlock & Continue to Milestone ${currentMilestone + 2} →` : 'Complete Reading & Unlock Final Reflections →'}</span>
                  </button>
                </div>
              </div>
            ` : `
              <div class="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-800 flex items-center gap-2">
                <span>🔒</span>
                <span class="font-bold">You cannot advance to the next reading section without answering this checkpoint.</span>
              </div>
            `}
          </div>
        `;

        // Render LOCKED FUTURE MILESTONES (Questions are hidden!)
        if (currentMilestone + 1 < checkpoints.length) {
          html += `
            <div class="space-y-2 pt-2">
              <span class="text-[10px] font-black uppercase text-slate-400 px-1">Upcoming Locked Milestones</span>
              ${checkpoints.slice(currentMilestone + 1).map((futureCp, fIdx) => `
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between opacity-60">
                  <span class="text-xs font-bold text-slate-500 flex items-center gap-2">
                    <span>🔒</span> <span>Milestone ${currentMilestone + 2 + fIdx}: ${futureCp.section || 'Next Reading Section'}</span>
                  </span>
                  <span class="text-[9px] font-black uppercase bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">
                    Complete Milestone ${currentMilestone + 1} to Unlock
                  </span>
                </div>
              `).join('')}
            </div>
      ...
      // Past Milestones Accordion (if any completed previously)
      const passedIndices = checkpoints.map((_, i) => i).filter(i => pdfCheckpointsCompletedMap[`${subId}_pdf_${i}`] && (allPdfCheckpointsPassed || i < currentMilestone));
      if (passedIndices.length > 0 && !allPdfCheckpointsPassed) {
        html += `
          <details class="group bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden mt-3">
            <summary class="p-3 text-xs font-bold text-slate-700 flex items-center justify-between cursor-pointer hover:bg-slate-100">
              <span class="flex items-center gap-2"><span>✓</span> <span>View Completed Reading Milestones (${passedIndices.length})</span></span>
              <span class="text-[10px] text-teal-700 font-bold group-open:hidden">Expand</span>
            </summary>
            <div class="p-4 border-t border-slate-200 space-y-3 bg-white">
              ${passedIndices.map(pIdx => {
                const pCp = checkpoints[pIdx];
                return `
                  <div class="p-3 bg-emerald-50/40 rounded-xl border border-emerald-200 text-xs space-y-1">
                    <div class="flex items-center justify-between">
                      <span class="font-black text-emerald-900">${pCp.section || `Milestone ${pIdx + 1}`}</span>
                      <span class="text-emerald-700 font-bold text-[10px]">Passed</span>
                    </div>
                    <p class="text-slate-800 font-bold">${pCp.question}</p>
                    <p class="text-emerald-700 text-[11px]">✓ Answer: ${pCp.choices?.[pCp.correctIndex] || ''}</p>
                  </div>
                `;
              }).join('')}
            </div>
      ...
      if (btnUnlock && lockText) {
        if (allPdfCheckpointsPassed) {
          ...
          lockText.innerText = `📖 Complete Milestone ${currentMilestone + 1} of ${checkpoints.length} to unlock final reflections.`;
        }
      }
    }

    function submitPDFSequentialAnswer(key, cpIdx, chIdx) {
      const steps = selectedSubmodule?.videoSteps || [];
      ...
      if (isCorrect) {
        ...
        showToast(`✓ Correct! Reading Milestone ${cpIdx + 1} mastered.`, 'emerald');
      } else {
        delete pdfCheckpointsCompletedMap[key];
        showToast(`✕ Incorrect. Review ${cp.section ? cp.section.split(':')[0] : 'the section'} and try again.`, 'rose');
      }
    ...
    function advancePDFMilestone(nextIdx) {
      const subId = selectedSubmodule.id;
      const steps = selectedSubmodule?.videoSteps || [];
      const currentStep = steps[activeVideoStepIndex] || selectedSubmodule;
      const checkpoints = currentStep?.pdfCheckpoints || currentStep?.embeddedCheckpoints || [];
      
      if (nextIdx < checkpoints.length) {
        activePdfMilestoneIndexMap[subId] = nextIdx;
        showToast(`Unlocked Milestone ${nextIdx + 1}!`, 'teal');
        renderStudentPDFView(currentStep);
      } else {
        // All completed!
        unlockPDFReflections();
        showToast(`🎉 All reading checkpoints completed! Final reflections unlocked.`, 'emerald');
        renderStudentPDFView(currentStep);
      }
    }
